...
export function QuestionnaireOutline({ questionnaire }: { questionnaire: Questionnaire }) {
  if (questionnaire.sections.length === 0) {
    return (
      <p className="text-txt-mute" data-testid="outline-empty">
      ...
      {questionnaire.sections.map((section, index) => (
        <section key={section.key} data-testid="outline-section">
          <div className="flex items-baseline gap-3">
            <span className="elvt-num text-txt-dim">{index + 1}</span>
            <h3 className="text-emphasis">{section.title}</h3>
            <span className="elvt-label text-txt-dim">
              {section.questions.length}{" "}
              {section.questions.length === 1 ? "question" : "questions"}
            </span>
          </div>
          <p className="mt-1 max-w-[70ch] text-txt-mute">{section.intent}</p>
              ...
              <tbody>
                {section.questions.map((question) => (
                  <tr key={question.key} data-testid="outline-question">
                    ...
                    <td className="text-txt-mute">{humanize(question.type)}</td>
                    <td className="text-txt-mute">{question.required ? "Yes" : "No"}</td>
                    <td className="max-w-[24ch] truncate text-txt-mute">
